...
def load_framing_data() -> pd.DataFrame:
    ...
    """
    # 1. 2024년, 2025년 CSV 파일을 각각 읽음    
    df_2024 = pd.read_csv(BASE_DIR / "Catcher" / "catcher-framing.csv")
    # 나중에 두 데이터를 합쳤을 때 어느 연도 데이터인지 구분하기 위해 'game_year'라는 열(컬럼)을 강제로 추가
    df_2024['game_year'] = 2024
    
    df_2025 = pd.read_csv(BASE_DIR / "Catcher" / "catcher-framing-2.csv")
    df_2025['game_year'] = 2025
    
    # 2. pd.concat을 사용해 두 데이터프레임을 위아래로 이어 붙임
    # ignore_index=True를 하면 합쳐진 데이터의 인덱스 번호가 0부터 다시 깔끔하게 매겨짐
    df = pd.concat([df_2024, df_2025], ignore_index=True)
    
    # 3. [데이터 클렌징] 컬럼명 통일
    # 프레이밍 데이터만 선수 ID 컬럼명이 'id'로 되어 있음
    # 나중에 다른 데이터(블로킹, OAA 등)와 엮기(JOIN) 위해 통일성 있게 'player_id'로 바꿔줌
    if 'id' in df.columns:
...
def load_blocking_data() -> pd.DataFrame:
    ...
    # 블로킹 데이터는 별도의 컬럼명 수정이나 결측치 처리가 필요 없을 정도로 깔끔해서 바로 반환
    return df
...
def load_oaa_data() -> pd.DataFrame:
    ...
    # 문제점: OAA 데이터의 일부 성공률 지표가 숫자(73.5)가 아니라 문자열("73.5%") 형태로 들어있음
    # 컴퓨터는 "73.5%"를 글자로 인식하므로 덧셈 뺄셈 같은 머신러닝 연산을 할 수 없음
    pct_cols = [
    ...
    for col in pct_cols:
        if col in df.columns:
            # 1단계: 글자 속에서 '%' 기호를 찾아 없애고(replace), 혹시 모를 양옆 공백을 지워줌(strip).
            df[col] = df[col].astype(str).str.replace('%', '').str.strip()
            # 2단계: 깔끔해진 글자를 진짜 숫자(float)로 바꿔줌. 
            # errors='coerce'는 숫자로 바꿀 수 없는 이상한 값이 있으면 강제로 빈칸(NaN)으로 만듦.
...
def load_bat_tracking_data() -> pd.DataFrame:
    ...
    """
    # 타구 추적 데이터는 이미 2024년과 2025년이 하나의 파일로 합쳐져 다운로드 되었음
    # 파일 용량이 매우 크지만 판다스는 똑같이 읽어올 수 있음
    df = pd.read_csv(BASE_DIR / "All 24 25 Player" / "statcast_bat_tracking_2024_2025.csv")
    
    # 3. [데이터 클렌징] 주자 상황 결측치(Null) 처리
    # 타구 데이터에는 1루(on_1b), 2루(on_2b), 3루(on_3b)에 주자가 있는지 적혀있음
    # 만약 베이스에 주자가 없다면 빈칸(Null/NaN)으로 저장되어 있음
    # 머신러닝 모델은 빈칸을 만나면 에러를 뿜기 때문에, 주자가 없다는 뜻으로 '0'을 채워 넣어줌
    runner_cols = ['on_1b', 'on_2b', 'on_3b']
    for col in runner_cols:
        if col in df.columns:
            # fillna(0): 빈칸을 모두 0으로 덮어씌움
